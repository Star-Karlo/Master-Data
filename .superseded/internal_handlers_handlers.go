...
func writeError(c *gin.Context, err error) {
	...
	case errors.Is(err, repository.ErrConflict), errors.Is(err, repository.ErrIMEITaken):
		// A duplicate IMEI is a conflict, not a server fault. Returning 500
		// told the caller to retry something that will never succeed, and hid
