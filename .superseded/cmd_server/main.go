...
import (
	...
	"github.com/karlo/masterdata-service/internal/platform/revocation"
...
func run() error {
	...
	trackerService := services.NewTrackerService(trackerRepo, truckRepo)
	...
	defer stopWatching()
	revocationChecker, _ := revocation.FromEnv(watchCtx, "masterdata", tokenLifetimeHint())
	...
	router := routes.Setup(routes.Deps{
		...
		Tracker:     handlers.NewTrackerHandler(trackerService),
		Customer:    handlers.NewCustomerHandler(repository.NewCustomerRepository(db)),
	})
