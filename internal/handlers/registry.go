...
var (
	...
	groupFields    = query.FieldSet{"name": "name", "createdAt": "createdAt"}
...
// @Security BearerAuth
// @Produce  json
// @Success  200 {object} map[string]interface{} "success, data []services.GroupView, meta"
// @Router   /vehicle-groups [get]
func (h *RegistryHandler) ListGroups(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	p, ok := params(c, groupFields)
	if !ok {
		return
	}
	rows, total, err := h.reg.ListGroups(c.Request.Context(), companyID, p)
	if err != nil {
		writeRegistryError(c, err)
		return
	}
...
// @Accept   json
// @Produce  json
// @Param    body body services.GroupInput true "Group"
// @Success  201 {object} services.GroupView
// @Router   /vehicle-groups [post]
func (h *RegistryHandler) CreateGroup(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	var in services.GroupInput
	if err := c.ShouldBindJSON(&in); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	g, err := h.reg.CreateGroup(c.Request.Context(), companyID, in)
	if err != nil {
		writeRegistryError(c, err)
		return
	}
...
// @Param    body body services.GroupInput true "Changed fields"
// @Success  200 {object} services.GroupView
// @Router   /vehicle-groups/{id} [put]
func (h *RegistryHandler) UpdateGroup(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	...
	g, err := h.reg.UpdateGroup(c.Request.Context(), companyID, c.Param("id"), in)
	if err != nil {
		writeRegistryError(c, err)
		return
	}
...
// @Param    id path string true "Group id"
// @Success  200 {object} map[string]interface{}
// @Router   /vehicle-groups/{id} [delete]
func (h *RegistryHandler) DeleteGroup(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	if err := h.reg.DeleteGroup(c.Request.Context(), companyID, c.Param("id")); err != nil {
		writeRegistryError(c, err)
		return
	}
	response.OKWithMessage(c, "Group removed.", nil)
}
...
// @Param    id path string true "Group id"
// @Success  200 {object} map[string]interface{} "success, data []services.VehicleView, meta"
// @Router   /vehicle-groups/{id}/members [get]
func (h *RegistryHandler) GroupMembers(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	...
	rows, total, err := h.reg.GroupMembers(c.Request.Context(), companyID, c.Param("id"), p)
	if err != nil {
		writeRegistryError(c, err)
		return
	}
...
// @Produce  json
// @Param    id   path string true "Group id"
// @Param    body body membersRequest true "The complete membership"
// @Success  200 {object} map[string]interface{}
// @Router   /vehicle-groups/{id}/members [put]
func (h *RegistryHandler) SetGroupMembers(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	var in membersRequest
	if err := c.ShouldBindJSON(&in); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
	if err := h.reg.SetGroupMembers(c.Request.Context(), companyID, c.Param("id"), in.VehicleIDs, actor(c)); err != nil {
		writeRegistryError(c, err)
		return
	}
	response.OKWithMessage(c, "Membership updated.", nil)
}
...
// @Produce  json
// @Param    id   path string true "Group id"
// @Param    body body memberRequest true "Vehicle"
// @Success  200 {object} map[string]interface{}
// @Router   /vehicle-groups/{id}/members [post]
func (h *RegistryHandler) AddGroupMember(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	...
	if err := h.reg.AddGroupMember(c.Request.Context(), companyID, c.Param("id"), in.VehicleID, actor(c)); err != nil {
		writeRegistryError(c, err)
		return
	}
	response.OKWithMessage(c, "Vehicle added.", nil)
}
...
// @Success  200 {object} map[string]interface{}
// @Router   /vehicle-groups/{id}/members/{vehicleId} [delete]
func (h *RegistryHandler) RemoveGroupMember(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	if err := h.reg.RemoveGroupMember(c.Request.Context(), companyID, c.Param("id"), c.Param("vehicleId")); err != nil {
		writeRegistryError(c, err)
		return
	}
	response.OKWithMessage(c, "Vehicle removed from group.", nil)
}
...
func (h *RegistryHandler) VerifyDocument(c *gin.Context) {
	companyID, ok := caller(c)
	if !ok {
		return
	}
	d, err := h.reg.VerifyDocument(c.Request.Context(), companyID, c.Param("id"), actor(c))
	if err != nil {
		writeRegistryError(c, err)
		return
	}
